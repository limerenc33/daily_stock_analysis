...
新下载的行情缓存会同时写入 provider、请求窗口、行数和首尾日期元数据。缓存再次命中时，结果中的数据源会显示为 `cache:yfinance` 或 `cache:stooq`；缺少元数据的旧缓存或元数据与 CSV 不一致时显示为 `cache:unknown`，不得据此声称原始提供方。
...
在不修改 `us_quality_momentum` 参数的前提下，另对 2026-01-01 至 2026-08-10 做了一次补充时间检验。行情请求从 2025-03-27 开始以覆盖 140 日 warm-up，76/76 个标的均由 `yfinance` 返回；原始结果为 [`data/us_validation/us_quality_momentum_holdout_2026.json`](../data/us_validation/us_quality_momentum_holdout_2026.json)。

| 股票池 | 周期/交易数 | 策略收益 | CAGR | Sharpe | 最大回撤 | 胜率 | SPY 收益 | 对 SPY 超额 | 跑赢基准数 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 大盘股 22 | 15 / 71 | -0.9764% | -1.6348% | -0.0241 | -10.2245% | 40.00% | +7.8962% | -8.8725pp | 0/5 |
| 跨行业 60 | 15 / 75 | +4.4281% | +7.5506% | 0.6065 | -7.4642% | 60.00% | +7.8962% | -3.4681pp | 0/5 |
| 行业 ETF（探索） | 15 / 68 | +12.8014% | +22.4301% | 2.4387 | -2.5541% | 73.33% | +7.8962% | +4.9052pp | 4/5 |

两个正式股票池均没有跑赢任何一个正式基准。20 bps 往返成本下，大盘股池在 5/10/20 日持有期分别收益 -3.1593%、-0.9764%、-3.3029%；跨行业池虽然分别收益 +2.0530%、+4.4281%、+0.7193%，但三个持有期都落后 SPY，且 50 bps 成本下均转为负收益。行业 ETF 在这段短窗口表现较好，但它在 2015-2025 长样本中的收益为 -6.8225%，因此只能视为市场阶段诊断，不能替换正式股票策略或据此追逐近期参数。

这段区间只有 15 个非重叠 10 日周期，低于 50 周期门槛，而且是在 2026 年数据已经存在后执行的补充历史检验，不是真正的前瞻证据。它不能单独证明策略失效，但与原综合验证一起，进一步否定了“当前策略已有效”或可以产生购买建议的说法。每日 paper trading 会继续积累真正按日到达的数据，达到预设周期门槛前保持研究状态。
