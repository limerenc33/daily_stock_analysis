...
> 版本边界（2026-08-11）：本页数值对应 `us_quality_momentum` 1.0。生产策略文件已升级为 2.0，移除无法从日线回放取得的估值/市值默认分，并改用可审计六维证据卡。旧结果不能用于证明 2.0 有效；2.0 在重新完成同等级历史与样本外验证前继续标记为未验证。
...
`us_quality_momentum` 2.0 使用趋势确认 25%、动量质量 20%、风险控制 20%、流动性容量 15%、相对强度 10%、数据质量 10%，不再让缺失的估值和市值默认分参与排名。它与下文曾经归档的训练候选 `us_quality_momentum_v2_rejected` 不是同一个策略版本。

2.0 已在相同的 2015-2025 Yahoo 数据、两个正式股票池、五个基准、三个持有期、两档成本和 2,000 次 bootstrap 下重新验证，综合门槛仍未通过。原始结果：[`research/us_strategy_results/us_quality_momentum_scorecard_v2_2015_2025.json`](../research/us_strategy_results/us_quality_momentum_scorecard_v2_2015_2025.json)。

| 股票池 | 对 SPY 收益 | 对 SPY 超额 | Sharpe | 最大回撤 | 正超额基准数 | 样本外多数为正 | 固定阶段稳健性 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 大盘股 22 | +159.0860% | +43.1542pp | 0.5315 | -41.3754% | 4/5 | 4/5 | 2/5 |
| 跨行业 60 | +84.2981% | -31.6338pp | 0.3859 | -44.0370% | 3/5 | 1/5 | 2/5 |

两个股票池的中位 Sharpe 为 0.4587，低于 0.5；跨行业池只有 1/3 个持有期同时盈利并跑赢 SPY，50 bps 往返成本下失败；10 个正式股票池×基准单元的 bootstrap 95% 置信区间下界全部不为正。因此 2.0 只能继续 paper trading，不得用于真实购买建议。
