...
- [新功能] 美股模拟策略升级为可审计六维证据卡，日报和 Pages 同步展示每只候选的选股理由、风险、失效条件与数据缺口。
- [测试] 使用 2015-2025 Yahoo 日线对美股六维证据卡重跑多股票池、多基准、成本压力和 bootstrap 验证，并保留未通过结论。
- [新功能] 美股模拟交易加入可配置网格止损止盈、盘中行情轮询、日线触发回补、成交推送与 Pages 持仓风控展示。
- [新功能] 美股模拟交易新增每日新闻、SEC 财报/披露和公开研报评级证据层，2.1 资讯调整仅从部署后的下一新周期生效，并在日报与 Pages 展示来源和分数影响。
- [改进] 美股模拟交易归档由运行脚本统一落盘，并保存资讯原文摘要、逐条分析、预期影响和影响期限，避免历史流水或来源细节丢失。
- [修复] 每日模拟任务运行前恢复 `paper-trading-state` 的历史归档目录，避免跨次运行只保留最新交易日快照。
- [修复] 美股模拟交易看板按最新收益快照实时计算跑赢基准数量，并让状态诊断独立记录组合盈利与基准超额进度。
- [修复] 美股模拟日报增加错峰补偿调度和重复交易日短路，看板优先读取状态分支并降级到 Pages 快照，避免 Actions 延迟导致账本与页面长期停更。
